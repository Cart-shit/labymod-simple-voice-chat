...
public class PlayerList extends WidgetBase {

    private static final ResourceLocation TEXTURE = new ResourceLocation(VoicechatClient.MODID, "textures/gui/gui_group.png");
    private static final ResourceLocation SPEAKER_OFF = new ResourceLocation(VoicechatClient.MODID, "textures/gui/speaker_off.png");
    private static final ResourceLocation SPEAKER = new ResourceLocation(VoicechatClient.MODID, "textures/gui/speaker.png");
    private static final ResourceLocation CHANGE_VOLUME = new ResourceLocation(VoicechatClient.MODID, "textures/gui/change_volume.png");
    private static final ResourceLocation DISCONNECT = new ResourceLocation(VoicechatClient.MODID, "textures/gui/disconnected.png");
    ...
    private final LabymodVoicechatClient voiceChatClient;

    public PlayerList(VoiceChatScreenBase screen, int posX, int posY, int xSize, int ySize, Supplier<List<PlayerState>> playerStates) {
    ...
    public void tick() {
        super.tick();
        if (!voiceChatClient.getState().isConnected()) {
            mc.setScreen(null);
        }
    }
    ...
    public void drawGuiContainerBackgroundLayer(PoseStack matrixStack, float partialTicks, int mouseX, int mouseY) {
        ...
        for (int i = getOffset(); i < entries.size() && i < getOffset() + columnCount; i++) {
            ...
            } else if (VoicechatClient.CLIENT.getTalkCache().isTalking(state.getGameProfile().getId())) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        for (int i = 0; i < hoverAreas.length; i++) {
            ...
            if (mouseX < guiLeft + xSize - 3 - 16) {
                if (!state.getGameProfile().getId().equals(mc.player.getUUID())) {
                    boolean muted = !state.isDisabled();
                    state.setDisabled(muted);
                    VoicechatClient.MUTED_PLAYER.setPlayerMuted(state.getGameProfile().getId(), muted);
                    VoicechatClient.MUTED_PLAYER.save();
                }
            } else {
                if (!state.getGameProfile().getId().equals(mc.player.getUUID())) {
